#!/usr/bin/env node
// App 发版：升级 app/package.json 与 app/app.json 的 version → 提交并打 tag app-v<version>。
// 用法：pnpm release:app <patch|minor|major> [--dry-run]
// 推送 tag 后，GitHub Actions 会自动构建并发布 Release（见 .github/workflows/build-release.yml）。
...
if (!['patch', 'minor', 'major'].includes(bump)) {
  console.error('用法：pnpm release:app <patch|minor|major> [--dry-run]')
  process.exit(1)
}
...
const ok = await confirm(`\n确认发布 ${newVer}（改版本号、提交、打 tag app-v${newVer}）？(y/N) `)
if (!ok) {
