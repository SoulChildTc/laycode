...
interface ServersContextValue {
  ...
  test: (entry: { host: string; port: number; token: string }) => Promise<boolean>
  connect: (entry: ServerEntry) => Promise<boolean>
...
export function ServersProvider({ children }: { children: React.ReactNode }) {
  ...
  const test = useCallback(async (entry: { host: string; port: number; token: string }) => {
    const client = new LayCodeClient(entry)
    return await client.health()
  }, [])

  const connect = useCallback(async (entry: ServerEntry) => {
    const client = new LayCodeClient({ host: entry.host, port: entry.port, token: entry.token })
    const ok = await client.health()
    if (ok) await AsyncStorage.setItem(LAST_KEY, entry.id)
    return ok
  }, [])
