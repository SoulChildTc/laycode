# 聊天页面架构与重构纲领（CHAT-ARCHITECTURE）

> 本文档沉淀对聊天页面（`app/src/screens/SessionScreen.tsx`）的勘探、考古与可行性核实结论，作为后续重构的行动纲领。所有关键结论均以一手来源（SDK 生成类型、opencode server 源码）为依据，不以二手整理文档为准。
>
> 状态：方向已定（跟随 opencode V2 主线），两个落地前提已用源码验证。尚未开始编码。
...
- **无自动化测试**：项目目前无单测，重构靠手动验证 + 保护清单。建议在领域层落地前至少为其 reducer 补纯函数单测（它是纯函数，易测）。
- **`effect` 库能否在 RN 复用**：官方 `memory()` adapter 依赖 `immer`（RN 可用）和 `effect`（需验证包体积/兼容性）。若不宜直接引入，按其公开逻辑重写纯 TS 版。
